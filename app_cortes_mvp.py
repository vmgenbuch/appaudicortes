...
SUCURSALES = [
    {
        "id": 1,
        "nombre": "Generales Revolución",
        "alias": ["revolucion", "generales revolucion"],
    },
    {
        "id": 2,
        "nombre": "Generales Galerías",
        "alias": ["galerias", "generales galerias"],
    },
    {
        "id": 3,
        "nombre": "Generales Linda Vista",
        "alias": ["linda vista", "generales linda vista", "GRLV"],
    },
    {
        "id": 4,
        "nombre": "Generales Barragán",
        "alias": ["barragan", "generales barragan"],
    },
    {
        "id": 5,
        "nombre": "Generales Santa Catarina",
        "alias": ["santa catarina", "generales santa catarina"],
    },
    {"id": 6, "nombre": "Casona Linda Vista", "alias": ["casona linda vista"]},
    {"id": 7, "nombre": "Casona Galerías", "alias": ["casona galerias", "csg"]},
    {
        "id": 8,
        "nombre": "Generales Sendero la Fe",
        "alias": ["sendero la fe", "sendero fe", "sendero"],
    },
    {"id": 9, "nombre": "Buchakas Citadel", "alias": ["citadel", "buchakas citadel"]},
    {"id": 10, "nombre": "Buchakas Esfera", "alias": ["esfera", "buchakas esfera"]},
    {
        "id": 11,
        "nombre": "Buchakas Interplaza",
        "alias": ["interplaza", "buchakas interplaza"],
    },
    {
        "id": 12,
        "nombre": "Buchakas San Roque",
        "alias": ["san roque", "buchakas san roque"],
    },
    ...
    {
        "id": 17,
        "nombre": "Buchakas Valle Oriente",
        "alias": ["valle oriente", "buchakas valle oriente"],
    },
]
...
def init_db() -> None:
    ...
    columnas = [
        row[1] for row in db.execute("PRAGMA table_info(cortes_subidos)").fetchall()
    ]
...
def leer_total_con_openai(path):
    try:
        ...
        response = client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "Extrae datos de tickets. Responde SOLO JSON."
                },
                {
                    "role": "user",
                    "content": [
                        {"type": "text", "text": "Extrae el TOTAL DE VENTAS de esta imagen. Devuelve JSON con: total_ventas"},
                        {
                            "type": "image_url",
                            "image_url": {
        ...
        print("RESPUESTA OPENAI:", contenido, flush=True)

        data = json.loads(contenido)

        return float(data.get("total_ventas"))

    except Exception as e:
        print("ERROR OPENAI OCR:", e, flush=True)
        return None
...
def leer_total_desde_imagen(path):
    try:
        ...
        for linea in lineas:
            ...
            if any(p in linea_lower for p in ["total", "suma", "ventas"]):
                numeros = re.findall(r"\d+(?:\.\d{2})?", linea.replace(",", ""))

                for n in numeros:
                    try:
                        valor = float(n)
                        if valor > 1000:  # evita ruido pequeño
                            posibles_totales.append(valor)
                    except:
                        continue

        if posibles_totales:
            return max(posibles_totales)

        # fallback (por si no encontró palabras clave)
        matches = re.findall(r"\d+(?:\.\d{2})?", texto.replace(",", ""))

        valores = [float(m) for m in matches if float(m) > 1000]
...
def save_uploaded_file(
    file_storage, sucursal: str, fecha: str, turno: str, slot: int
) -> Optional[str]:
    try:
        if not file_storage:
        ...
        if not allowed_file(file_storage.filename):
            raise ValueError(
                f"Archivo no permitido en slot {slot}: {file_storage.filename}"
            )
...
def fetch_all_cortes():
    ...
    rows = db.execute(
        """
        SELECT id, sucursal, fecha, turno, cajera, total_corte, total_ocr, observaciones,
...
def update_status(corte_id: int, status: str) -> None:
    db = get_db()
    processed_at = (
        datetime.now().strftime("%Y-%m-%d %H:%M:%S") if status != "pendiente" else None
    )
    db.execute(
...
def resolver_pagos_ticket(
    pagos: list, total_ticket: float, tolerancia: float = 0.01
) -> list:
    if not pagos:
        return []
...
def fetch_zetus_por_sucursal(nombre_sucursal: str):
    try:
        ...
        return [{"sucursal": nombre_sucursal, "fecha": AYER, "total": total}]
    except Exception as e:
...
def api_comparar_linda_vista():
    ...
    cortes_app = [
        c
        for c in cortes_app
        if c["sucursal_nombre"] == "Generales Linda Vista" and c["fecha"] == AYER
    ]
...
def api_comparar_casona_galerias():
    ...
    cortes_app = [
        c
        for c in cortes_app
        if c["sucursal_nombre"] == "Casona Galerías" and c["fecha"] == AYER
    ]
...
def api_crear_corte():
    try:
        ...
        if not sucursal or not fecha or not turno:
            return (
                jsonify(
                    {"ok": False, "error": "Sucursal, fecha y turno son obligatorios"}
                ),
                400,
            )
...
def api_subir_imagen_corte(corte_id):
    try:
        ...
        if tesseract_path:
            ...
            print("Tesseract encontrado en:", tesseract_path, flush=True)
        else:
            print("Tesseract NO encontrado en PATH", flush=True)

        total_ocr = None
        totales_detectados = []

        print("IMG1 VALUE:", img1, flush=True)
        print("IMG2 VALUE:", img2, flush=True)

        if img1:
            ruta1 = UPLOAD_DIR / img1
            print("Procesando OCR imagen 1 en:", ruta1, flush=True)
            total1 = leer_total_con_openai(ruta1) or leer_total_desde_imagen(ruta1)
            if total1:
                totales_detectados.append(total1)

        if img2:
            ruta2 = UPLOAD_DIR / img2
            print("Procesando OCR imagen 2 en:", ruta2, flush=True)
            total2 = leer_total_desde_imagen(ruta2)
            if total2:
                totales_detectados.append(total2)

        if totales_detectados:
            total_ocr = max(totales_detectados)
        ...
        return jsonify(
            {"ok": True, "imagen_1": img1, "imagen_2": img2, "total_ocr": total_ocr}
        )
