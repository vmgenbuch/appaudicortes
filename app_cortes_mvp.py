...
def leer_total_con_openai(path):
    try:
        print("ENTRANDO A OPENAI OCR", flush=True)
        ...
        response = client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "Eres un sistema OCR de tickets. Responde únicamente JSON válido."
                },
                {
                    "role": "user",
                    "content": [
                        {
                            "type": "text",
                            "text": 'Extrae el TOTAL DE VENTAS de esta imagen. Devuelve SOLO JSON así: {"total_ventas": 12345.67}'
                        },
                        {
                            "type": "image_url",
                            "image_url": {
        ...
        total = data.get("total_ventas")

        if total is None:
            print("NO SE ENCONTRO total_ventas EN RESPUESTA", flush=True)
            return None

        return float(total)

    except Exception as e:
        print("ERROR OPENAI OCR:", repr(e), flush=True)
        return None
...
def api_subir_imagen_corte(corte_id):
    try:
        ...
        # =========================
        # GUARDAR IMAGEN 2 (OPCIONAL)
        # =========================
        file2 = request.files.get("imagen_2")
        img2 = None
        ...
        print("ANTES UPDATE IMAGENES", flush=True)
        ...
        try:
            ...
            print("ERROR EN UPDATE:", repr(e), flush=True)
            raise

        print("ANTES COMMIT IMAGENES", flush=True)

        try:
            db.commit()
            print("COMMIT OK", flush=True)
        except Exception as e:
            print("ERROR EN COMMIT:", repr(e), flush=True)
            raise

        print("DESPUES COMMIT IMAGENES - ANTES OCR", flush=True)
        ...
        if img1:
            ...
            print("Procesando OpenAI OCR imagen 1 en:", ruta1, flush=True)
            total1 = leer_total_con_openai(ruta1)
        ...
        if img2:
            ...
            print("Procesando OpenAI OCR imagen 2 en:", ruta2, flush=True)
            total2 = leer_total_con_openai(ruta2)
